...
核心指标 (OEC - Overall Evaluation Criteria)：1日留存 (Retention_1) & 7日留存 (Retention_7)选择原理：留存率是衡量“关卡难度调整”对用户粘性影响的最直接指标。根据产品假设，推迟障碍（Delay Gratification）旨在减少早期挫败感，从而提升短期与中期的用户留存，直接关联 LTV（生命周期价值）。

辅助/行为指标 (Secondary Metrics)：游戏回合数 (Sum_GameRounds)选择原理：用于监测用户的活跃深度。我们不仅关注用户“回不回来”（留存），还关注用户“玩得久不久”（活跃）。该指标能反映新策略是否在提升留存的同时，维持了用户的游戏热情，而非导致用户“虽然留下来了，但因为关卡太简单感到无聊而减少游玩”。
...
操作方式：利用统计学公式（或 Python statsmodels）计算得出，要检测出上述 MDE，单组样本量需达到约 1.6 万人。数据现状：本数据集包含 90,189 名用户（每组约 4.5 万），远超理论最小样本量，这保证了实验具备极高的统计效力。
